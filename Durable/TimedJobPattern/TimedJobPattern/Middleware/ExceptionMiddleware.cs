...
public class ExceptionMiddleware : IFunctionsWorkerMiddleware
{
    ...
    public async Task Invoke(FunctionContext context, FunctionExecutionDelegate next)
    {
        ...
        catch (Exception ex)
        {
            ...
            MyExceptionHandlerMessage exceptionMessage = new MyExceptionHandlerMessage();
            // Aggregate Exception handling: https://learn.microsoft.com/en-us/dotnet/api/system.aggregateexception.handle?view=netframework-4.7.2#examples
            if (ex is AggregateException)
            {
                var aggregate = (AggregateException)ex;
                aggregate.Handle(ex =>
                {
                    exceptionMessage = WrapTheException(ex);
                    return true;
                });
            }
            else
